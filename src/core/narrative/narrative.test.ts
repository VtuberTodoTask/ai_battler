...
describe('Narrative generation', () => {
  it('generates one record per explicit call and updates candidate', async () => {
    ...
    expect(record.promptVersion).toBe('v9')
  })
...
describe('Narrative prompt integrity', () => {
  it('expedition prompt includes request, party, and writing instructions', () => {
    ...
    expect(user).toContain('Omitted Beat IDs:')
    expect(user).toContain('NARRATIVE FOCUS')
    expect(user).toContain('MONTAGE は 1～3 文程度')
    expect(user).toContain('TIMELINE のすべての出来事を順番に説明しない')
    expect(user).toContain('同じ結果を繰り返し説明しない')
    expect(user).toContain('水筒を渡す')
    expect(user).toContain('不在の出来事')
    expect(user).toContain('Omitted Beat IDs')
    expect(user).not.toContain('目立った消耗はない')
    expect(user).not.toContain('手当てが行われなかった')

    expect(system).toContain('FACT PRESERVATION')
    expect(system).toContain('FACT COVERAGE')
    expect(system).toContain('不在は出来事ではない')
    expect(system).toContain('手当ては行われなかった')
    expect(system).toContain('目立った消耗はなかった')
    expect(system).toContain('キャラクター特性は傾向である')
    expect(system).toContain('関係性の差異化')
    expect(system).toContain('UI が担当')
  ...
  it('expedition prompt instructs the AI not to speak or decide for the player', () => {
    ...
    expect(user).toContain('TIMELINE に書かれていない')
    expect(user).toContain('店主の反応')
    expect(user).toContain('Party Member 側の台詞')
  })
