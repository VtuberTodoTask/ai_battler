...
/**
 * Phase 9.7 World Event reference. Present only on the one daily request
 * generated by an active World Event — never together with `chain` (a
 * request is always either an ordinary standalone request, a Quest Chain
 * follow-up, or a World Event request, by construction of how each is
 * built; never more than one of `chain`/`worldEvent`).
 */
export interface TavernWorldEventRef {
  eventId: string
  definitionId: string
  dayIndex: 1 | 2 | 3
  totalDays: 3
...
export interface TavernRequestOffer {
  ...
  worldEvent?: TavernWorldEventRef
}
