...
import {
  ...
  createWorldEventLogSceneInput,
  type WorldEventLogRowViewModel,
  type WorldEventLogSceneInput,
  type WorldEventLogViewModel,
} from '../../viewModel/worldEventLogViewModel.ts'
...
export class WorldEventLogScene implements GameScene {
  readonly id = 'worldEventLog'
  ...
  private _input: WorldEventLogSceneInput | undefined = undefined
  private _viewModel: WorldEventLogViewModel | null = null
  ...
  mount(context: GameSceneContext, input?: unknown): void {
    ...
    this._input =
      (input as WorldEventLogSceneInput | undefined) ??
      createWorldEventLogSceneInput({ sceneId: 'tavern' })
  ...
  private updateViewModel(): void {
    ...
    this._viewModel = buildWorldEventLogViewModel(
      this._campaign,
  ...
  private render(context: GameSceneContext): void {
    ...
    const titleLabel = new GameLabel('世界情勢', theme, 'heading')
    ...
    if (rows.length === 0) {
      ...
      for (const row of rows) {
        ...
        const listRow = new TavernListRow({
          ...
          subtitle: `${row.statusLabel} / ${row.progressLabel} / ${row.startedDayLabel}`,
          selected: expanded,
        })
        listRow.y = y
        ...
        if (expanded) {
          const detailLines = [
            row.startedDayLabel,
            row.description,
            `対応状況 ${row.progressLabel}`,
            `予定期間 ${row.periodLabel}`,
          ]
          if (row.endedLabel) {
            detailLines.push(row.endedLabel)
          }
          for (const line of detailLines) {
            const detailRow = new TavernListRow({
              width: rowWidth - 24,
            ...
            detailRow.x = 24
            detailRow.y = y
            scroll.content.addChild(detailRow)
            y += detailRowHeight + 4
          }
          y += gap
        }
