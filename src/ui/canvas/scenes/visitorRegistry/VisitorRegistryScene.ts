...
import {
  ...
  type VisitorRegistryRowViewModel,
  type VisitorRegistrySceneInput,
  type VisitorRegistryViewModel,
...
export class VisitorRegistryScene implements GameScene {
  readonly id = 'visitorRegistry'
  ...
  private _input: VisitorRegistrySceneInput | undefined = undefined
  private _viewModel: VisitorRegistryViewModel | null = null
  ...
  mount(context: GameSceneContext, input?: unknown): void {
    ...
    this._input =
      (input as VisitorRegistrySceneInput | undefined) ??
      createVisitorRegistrySceneInput({ sceneId: 'tavern' })
  ...
  private updateViewModel(): void {
    ...
    this._viewModel = buildVisitorRegistryViewModel(
      this._campaign,
  ...
  private render(context: GameSceneContext): void {
    ...
    const titleLabel = new GameLabel('来訪者台帳', theme, 'heading')
    ...
    if (rows.length === 0) {
      ...
      for (const row of rows) {
        const listRow = new TavernListRow({
          ...
          theme,
          title: row.name,
          subtitle: `${row.statusLabel} · ${row.visitCountLabel} · ${row.lastArrivalDayLabel}`,
        })
        listRow.y = y
        listRow.onActivate = () => this.openPartyDetail(row.id)
  ...
  private openPartyDetail(partyId: string): void {
    if (!this._context) return
    const input: PartyDetailSceneInput = {
      partyId,
      returnTarget: {
        sceneId: 'visitorRegistry',
