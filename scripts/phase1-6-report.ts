...
const seedArg = process.argv[2] ?? 'phase1-6-ability-cost-v1'
...
function runBenchmark(rank: 'E' | 'D' | 'C' | 'B' | 'A' | 'S', trials: number) {
  ...
  for (let t = 0; t < trials; t++) {
    ...
    const budget =
      partyThreat *
      ({ easy: 1.25, normal: 1.56, hard: 2, deadly: 2.5 } as const)[
        benchmarkConfig.difficulty
      ]
    const encounterMult = { standard: 1, eliteGroup: 1, swarm: 1, boss: 1 }[
      shape
    ]
    const slotTarget = budget / enemies.length / encounterMult

    for (const enemy of enemies) {
      const rankScore = { E: 1, D: 2, C: 4, B: 7, A: 11, S: 16, DISASTER: 24 }[
        enemy.rank
      ]
  ...
  return {
    ...
    avgEnemyRank: totalRankScore / enemyCount,
    avgBodyScale: totalBodyScale / enemyCount,
    minBodyScale,
    maxBodyScale,
    avgAbilityCost: totalAbilityThreat / enemyCount,
    avgActualThreat: totalActualThreat / enemyCount,
...
function main() {
  const lines: string[] = []
  lines.push(`=== Phase 1.6 Ability Cost Benchmark ===`)
  ...
  for (const rank of benchmarkConfig.ranks) {
    ...
    lines.push(
      `   rounds=${r.avgRounds.toFixed(1)} enemies=${r.avgEnemyCount.toFixed(2)} hp=${r.avgEnemyTotalHp.toFixed(1)} abilities=${r.avgEnemyAbilityCount.toFixed(2)} enemyActions=${r.avgEnemyActions.toFixed(2)} rawRatio=${r.avgRawRatio.toFixed(2)} adjRatio=${r.avgAdjRatio.toFixed(2)}`,
    )
    lines.push(
      `   avgRank=${r.avgEnemyRank.toFixed(2)} bodyScale=${r.avgBodyScale.toFixed(3)} minBodyScale=${r.minBodyScale.toFixed(3)} maxBodyScale=${r.maxBodyScale.toFixed(3)} abilityCost=${r.avgAbilityCost.toFixed(3)} actualThreat=${r.avgActualThreat.toFixed(3)} targetThreat=${r.avgTargetThreat.toFixed(3)} threatError=${r.avgThreatError.toFixed(3)}`,
    )
  ...
  const fileName = `phase1-6-report-${seedArg}.txt`
